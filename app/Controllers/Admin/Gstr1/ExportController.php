...
use App\Models\gstr1\export\ExportModel;
use App\Models\gstr1\export\ExportItemDetailsModel;
use Exception;
...
class ExportController extends AdminBaseController {
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                $result = $this->addDataToDB('6a_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('6a_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/export/' . $question_id);
        } else {
         ...
         //  echo '<pre>'; print_r($data);die;
            ...
            $data['file_name'] = 'gstr1/export-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new ExportModel();
                $secondaryModel = new ExportItemDetailsModel();
                $this->removeData('6a_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/export/' . $question_id);
            }
        }
