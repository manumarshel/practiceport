        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <h5>B2C(Large) Invoices - Add Invoice</h5>
                </div>
                <div class="m-t-30">
                    <form id="form_validation" action="<?= base_url().'/admin/gstr1/b2cl/add-invoice/'.$question_id; ?>" method="post"  class="form">
                        ...
                        <div class="row">
                            <div class="col form-group">
                                ...
                                <select name="eligible_to_taxed" id="" class="form-control">
                                    <option value="1" <?= set_value('eligible_to_taxed', ((!empty($form_data)) ? (($form_data['is_eligible'] == 1) ? 'selected' : '') : '')); ?>>Yes</option>
                                    <option value=0" <?= set_value('eligible_to_taxed', ((!empty($form_data)) ? (($form_data['is_eligible'] == 0) ? 'selected' : '') : '')); ?>>No</option>
                                </select>
                            </div>
                        </div>
                        <div class="row">
...
<!--                                <input name="pos" type="text" value="--><?//= set_value('pos', ((!empty($form_data['pos'])) ? $form_data['pos'] : '')); ?><!--"  class="form-control" placeholder="">-->
                            </div>
                            <div class="col form-group">
                                <label  for="">Invoice no.</label>
                                <input name="invoice_number" value="<?= set_value('invoice_number', ((!empty($form_data['invoice_no'])) ? $form_data['invoice_no'] : '')); ?>" type="text" class="form-control" placeholder="">
                            </div>
                            <div class="col form-group">
