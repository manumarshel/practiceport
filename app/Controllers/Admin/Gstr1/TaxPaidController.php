...
class TaxPaidController extends AdminBaseController {

     public function index() {
        helper(['common']);
        $Model = new TaxpaidModel();
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            return redirect()->to('admin/gstr1/tax-paid/' . $question_id);
        } else {
            $data['pos_list'] = get_pos_list();
            ...
            $data['file_name'] = 'gstr1/taxpaid-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new TaxpaidModel();
                $secondaryModel = new TaxpaidItemDetailsModel();
