...
//echo $questionId; exit;
      ...
      $data['ip_c'] = $qstn['ip_contribution'];
        $data['emp_c']= $qstn['emp_contribution'];
        $data['total']=  $data['ip_c'] + $data['emp_c'];
        
        $data['emp_code']= $qstn['emp_code'];
    ...
    public function login()
    {
       ...
       $companyId = $qstn['company_id'];
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($_POST['txtChallanCaptcha'] != "9cb5201") {
                $data['capError'] = 'Invalid Captcha';

                return view('esi/login', $data);
            }

            if ($cmpny['username'] != $_POST['txtUserName'] || $cmpny['password'] != $_POST['txtPassword']) {
                $data['authError'] = 'Authentication Failed.4';

                return view('esi/login', $data);
            }

            $this->setUserSession($cmpny, $questionId);
            return redirect()->to('esi/dashboard');
         }
