...
<div class="row">
        ...
        <div class="card h-100">
            <div class="card-body">
                <div class="card-header-clean">
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge-soft badge-soft-primary p-2 rounded-3">
                            <i class="anticon anticon-usergroup-add font-size-16"></i>
                        </span>
                        <div>
                            <h5 class="mb-0">Latest Registered Students</h5>
                        ...
                        <tbody>
                            <?php if (!empty($users)): ?>
                                <?php $idx = 1; foreach ($users as $u): ?>
                                    <tr>
                                        <td class="font-weight-semibold text-muted">#<?= $idx++; ?></td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="user-avatar-badge mr-3" style="background: <?= in_array($idx % 3, [0, 1]) ? 'linear-gradient(135deg, #3b82f6, #1d4ed8)' : 'linear-gradient(135deg, #10b981, #047857)' ?>;">
                                                    <?= strtoupper(substr($u['first_name'] ?? 'U', 0, 1) . substr($u['last_name'] ?? '', 0, 1)) ?>
                                                </div>
                                                <div>
                                                    <span class="font-weight-bold text-dark d-block" style="font-size: 13.5px;"><?= esc($u['first_name'] . ' ' . $u['last_name']); ?></span>
    ...
    <!-- RIGHT: RECENT ASSESSMENT SUBMISSIONS ACTIVITY STREAM -->
    <div class="col-lg-5 mb-4">
        <div class="card h-100">
            <div class="card-body">
                <div class="card-header-clean">
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge-soft badge-soft-success p-2 rounded-3">
                            <i class="anticon anticon-form font-size-16"></i>
                        </span>
                        <div>
                            <h5 class="mb-0">Assessment Activity</h5>
                            ...
                            <?php foreach ($recent_assessments as $asmt): ?>
                                <div class="list-group-item px-0 py-3 border-bottom d-flex justify-content-between align-items-center" style="border-color: #f1f5f9 !important;">
                                    <div class="d-flex align-items-center mr-2">
                                        <div class="user-avatar-badge mr-3" style="background: linear-gradient(135deg, #8b5cf6, #6d28d9); width: 36px; height: 36px;">
                                            <i class="anticon anticon-file-excel font-size-16"></i>
                                        </div>
                                        <div>
                                            ...
                                            <small class="text-muted d-block">
                                                <?= esc($asmt['course_name'] ?: 'Assessment') ?> &bull; 
                                                <a href="<?= base_url('public/assets/uploads/assessments/' . $asmt['answer_file']) ?>" target="_blank" class="text-primary font-weight-semibold">
                                                    <?= esc($asmt['answer_file']) ?>
                                                </a>
                                            </small>
                                        </div>
                                    </div>
                                    <div class="text-right flex-shrink-0" style="text-align: right;">
                                        ...
                                        <div class="text-muted mt-1" style="font-size: 11px;">
                                            <?= date('M d', strtotime($asmt['submitted_at'] ?? 'now')) ?>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
