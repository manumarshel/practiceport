...
use App\Models\gstr1\export_amendment\Export_amendmentItemDetailsModel;
use Exception;

class ExportAmendmentController extends AdminBaseController {
    ...
    public function index() {
        ...
        $data['file_name'] = 'gstr1/export-amendment';
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                $result = $this->addDataToDB('9a_exports_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('9a_exports_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/export-amendment/' . $question_id);
        } else {
            ...
            $data['file_name'] = 'gstr1/export-amendment-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new Export_amendmentModel();
                $secondaryModel = new Export_amendmentItemDetailsModel();
                $this->removeData('9a_exports_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/export-amendment/' . $question_id);
            }
        }
