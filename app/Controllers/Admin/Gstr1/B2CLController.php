...
class B2CLController extends AdminBaseController {

    public function index() {
        helper(['common']);
        $B2clModel = new B2clModel();
        $data['question_id'] = $this->request->uri->getSegment(4);
        $data['data_list'] = $B2clModel->select('*')->where('question_id', $data['question_id'])->get()->getResult();
        $data['file_name'] = 'gstr1/b2cl';
...
//        exit;
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                 $result = $this->addDataToDB('b2c_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('b2c_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/b2cl/' . $question_id);
        } else {
            ...
            $data['file_name'] = 'gstr1/b2cl-add-invoice';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new B2clModel();
                $secondaryModel = new B2clItemDetailsModel();
                $this->removeData('b2c_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/b2cl/' . $question_id);
            }
        }
