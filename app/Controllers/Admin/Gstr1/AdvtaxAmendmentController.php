...
use App\Models\gstr1\adv_tax_amendment\AdvtaxAmendmentItemDetailsModel;
use Exception;

class AdvtaxAmendmentController extends AdminBaseController {

    public function index() {
        helper(['common']);
        $Model = new AdvtaxAmendmentModel();
        ...
        $data['file_name'] = 'gstr1/advtax-amendment';
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                $result = $this->addDataToDB('atl_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('atl_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/adv-tax-amendment/' . $question_id);
        } else {
            $data['pos_list'] = get_pos_list();
            $data['financial_period_list'] = get_financial_period();
            $data['quarter_list'] = get_quarter_list();
          ...
          //  echo '<pre>';            print_r($data);die;
            ...
            $data['file_name'] = 'gstr1/advtax-amendment-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new AdvtaxAmendmentModel();
                $secondaryModel = new AdvtaxAmendmentItemDetailsModel();
                $this->removeData('atl_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/adv-tax-amendment/' . $question_id);
            }
        }
