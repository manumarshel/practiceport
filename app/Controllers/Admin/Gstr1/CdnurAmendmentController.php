...
use App\Models\gstr1\cdnur_amendment\Cdnur_amendmentItemDetailsModel;
use Exception;

class CdnurAmendmentController extends AdminBaseController {
    ...
    public function index() {
        ...
        $data['file_name'] = 'gstr1/cdnur-amendment';
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                $result = $this->addDataToDB('9c_cdnur_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('9c_cdnur_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/cdnur-amendment/' . $question_id);
        } else {
            ...
            $data['file_name'] = 'gstr1/cdnur-amendment-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new Cdnur_amendmentModel();
                $secondaryModel = new Cdnur_amendmentItemDetailsModel();
                $this->removeData('9c_cdnur_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/cdnur-amendment/' . $question_id);
            }
        }
