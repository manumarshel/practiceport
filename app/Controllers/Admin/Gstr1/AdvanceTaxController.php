...
use App\Models\gstr1\adv_tax\AdvtaxItemDetailsModel;
use Exception;

class AdvanceTaxController extends AdminBaseController {

    public function index() {
        helper(['common']);
        $Model = new AdvtaxModel();
    ...
    public function add() {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($pk_Id > 0) {
                $result = $this->addDataToDB('11a_tax_id', $formData, $primary_Model, $Idm, $pk_Id);
            } else {
                $result = $this->addDataToDB('11a_tax_id', $formData, $primary_Model, $Idm);
            }

            $session->setFlashdata('success', 'Action has successfully completed');
            return redirect()->to('admin/gstr1/adv-tax/' . $question_id);
        } else {
            $data['pos_list'] = get_pos_list();
            ...
            $data['file_name'] = 'gstr1/advtax-liability-add';
            return $this->admin_layout($data);
        }
    }
    ...
    public function remove() {
        if ($this->request->getMethod() == 'post') {
            if ($this->request->getPost('question_id') && $this->request->getPost('pk_id')) {
                ...
                $primaryModel = new AdvtaxModel();
                $secondaryModel = new AdvtaxItemDetailsModel();
                $this->removeData('11a_tax_id', $question_id, $pk_id, $primaryModel, $secondaryModel);
                $session->setFlashdata('success', 'Action has successfully completed');
                return redirect()->to('admin/gstr1/adv-tax/' . $question_id);
            }
        }
